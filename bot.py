...
async def profile_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        dialog.user["black_flags"] = text
        await send_text(update, context, "Каковы цели знакомства?")
    elif dialog.count == 5:
        dialog.user["goals"] = text
        prompt = load_prompt("profile")
        user_info = dialog_user_info_to_str(dialog.user)
        my_message = await send_text(update, context, "ChatGPT генерирует профиль...")
...
async def opener_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        user_info = dialog_user_info_to_str(dialog.user)
        my_message = await send_text(update, context, "ChatGPT генерирует первое сообщение...")
        answer = await chatgpt.send_question(prompt, user_info)
        await my_message.edit_text(answer)
